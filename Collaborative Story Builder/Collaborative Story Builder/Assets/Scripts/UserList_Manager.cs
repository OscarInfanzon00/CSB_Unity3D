...
public class UserList_Manager : MonoBehaviour
{
    ...
    private List<(string userID, string username)> allUsers = new List<(string, string)>(); // 🆕 Store all users

    void Start()
    {
        openUserListButton.onClick.AddListener(OpenUserList);
        closeButton.onClick.AddListener(CloseMenu);
        searchField.onValueChanged.AddListener(FilterUserList); // 🆕 Add search listener
    ...
    {
        allUsers.Clear(); // 🆕 Clear stored users before fetching new ones
        ...
        db.Collection("Users").GetSnapshotAsync().ContinueWithOnMainThread(task =>
        {
            if (task.IsCompletedSuccessfully)
            {
                ...
                foreach (DocumentSnapshot doc in task.Result.Documents)
                {
                    try
                    {
                        ...
                        if (data.ContainsKey("username") && data.ContainsKey("userID"))
                        {
                            ...
                            {
                                allUsers.Add((userID, username)); // 🆕 Store user in list
                            }
                        }
                        else
                        {
                ...
                // 🆕 Display filtered users (initially all users)
    ...
    private void FilterUserList(string searchText)
    {
        ...
        {
            if (user.username.ToLower().Contains(searchLower)) // Case-insensitive search
            {
